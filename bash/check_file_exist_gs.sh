...
###
###   Purpose:   
###   started:   2019-03-01 11:51:26 (pvr)
###
### ###################################################################### ###
...
if test "$INPUT" == ""; then
  usage $SCRIPT "-f INPUT not defined" "$SCRIPT -f <INPUT>"
fi

### # Continue to put your code here
cat $INPUT | grep -v '^#' | cut -d'=' -f 2 | grep -v '^$' | while read f
do
  log_msg 'MAIN' "Checking file: $f"
  check_exist_fail $f
done
